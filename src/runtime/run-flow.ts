...
import { defaultConfigFor, getNodeKind, kindIds } from "../registry/registry";
import { nodeConfig, resolveNodePorts, resolvePortSpec } from "../registry/ports";
...
export async function runFlow(
  ...
  // Built ONCE. Used only to explain an undelivered edge, but building it per
  // node would make a diagnostic quietly O(n^2) on the hot path.
  ...
  try {
    for (const node of order) {
      ...
      //
      // Checked HERE, before the activity gate, because the two outcomes are
      // both silent and only one reaches `collectInputs`. If the bad edge is the
      // node's only inbound one the node is SKIPPED; if it has another live edge
      // it RUNS with that port simply missing -- and then a downstream template
      // that is entirely correct renders empty, because the payload never
      // arrived to have a field in it.
      //
      // Keyed on the source having COMPLETED and on the handle being a port the
      // source could NEVER publish. A branch that was not taken is ordinary and
      // must never warn; a source that finished and cannot publish this port is
      // a misconfiguration that delivers nothing on any run. A warning that fires on
      // ordinary branching is noise, and noise is how a real warning stops being
      // read. PHP has always said this; this runtime ran the same graph and
      // said nothing (fancy-flow#17). Pinned by `flow/run-diagnostics`.
